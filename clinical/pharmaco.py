...
def paracetamol(poids: float) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    if poids >= PARA_POIDS_PIVOT_KG:
        return {
            "dose_g": PARA_DOSE_FIXE_G,
            "vol": "100 ml NaCl 0.9% sur 15 min",
            "freq": "Toutes les 6h (max 4g/24h)",
            "ref": "BCFI — Paracétamol IV",
        }, None
    dg = min(round(PARA_DOSE_KG * poids / 1000, 2), 1.0)
    return {
        "dose_g": dg,
        "vol": f"{dg*1000:.0f} mg dans 100 ml NaCl 0.9%",
        "freq": "Toutes les 6h",
        "ref": "BCFI — Paracétamol IV",
    }, None
...
def ketorolac(poids: float, atcd: list) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    return {
        "dose_mg": d,
        "admin": "IV lent 15s",
        "freq": "Toutes 6h — max 5j",
        "ref": "BCFI — Kétorolac (Taradyl)",
    }, None

def tramadol(poids: float, atcd: list, age: float) -> Tuple[Optional[Dict], Optional[str]]:
    alerts = []
    if "Épilepsie" in atcd:
        alerts.append("CONTRE-INDIQUÉ — Épilepsie (seuil épileptogène)")
    if "IMAO (inhibiteurs MAO)" in atcd:
        alerts.append("CONTRE-INDICATION ABSOLUE — IMAO")
    if "Antidépresseurs ISRS/IRSNA" in atcd:
        alerts.append("INTERACTION MAJEURE — ISRS/IRSNA")
    d = 100.0 if poids >= 50 else round(1.5 * poids, 0)
    return {
        "dose_mg": d,
        "admin": f"{d:.0f} mg dans 100 ml NaCl 0.9% — IV 30 min",
        "freq": "Toutes 6h (max 400 mg/24h)",
        "alertes": alerts,
        "ref": "BCFI — Tramadol",
    }, None
...
def piritramide(poids: float, age: float, atcd: list) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    return {
        ...
        "note": note,
        "ref": "BCFI — Piritramide (Dipidolor)",
    }, None
...
def morphine(poids: float, age: float) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    return {
        ...
        "admin": (
            f"IV lent 2-3 min — titrer par paliers {MORPH_PALIER_MG:.0f} mg / 5-10 min"
        ),
        "ref": "BCFI — Morphine",
    }, None
...
def naloxone(poids: float, age: float, dep: bool = False) -> Tuple[Optional[Dict], Optional[str]]:
    if dep:
        d = NALOO_DEP_MG
        a = f"{NALOO_DEP_MG}mg IV/2min — titration douce"
        n = "Dépendance — objectif ventilation"
    elif age < 18:
        d = min(round(NALOO_PED_KG * poids, 3), NALOO_ADULTE_MG)
        a = f"{d}mg IV (0.01mg/kg)"
        n = f"Pédiatrique {d} mg pour {poids} kg"
    else:
        d = NALOO_ADULTE_MG
        a = f"{NALOO_ADULTE_MG}mg IV — répéter 2-3 min (max 10mg)"
        n = ""
    return {
        "dose": d,
        "admin": a,
        "note": n,
        "surv": "Monitor SpO2+FR — demi-vie courte 30-90 min",
        "ref": "BCFI — Naloxone",
    }, None
...
def adrenaline(poids: float) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    d = (
        0.5
        if poids >= ADRE_POIDS_ADULTE_KG
        else min(round(ADRE_DOSE_KG * poids, 3), ADRE_DOSE_ADULTE_MG)
    )
    return {
        "dose_mg": d,
        "voie": "IM face antéro-latérale cuisse",
        "note": "0.5 ml sol 1mg/ml" if poids >= 30 else f"0.01mg/kg = {d}mg",
...
def glucose(poids: float, gl: Optional[float] = None) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    return {
        "dose_g": dg,
        "vol": f"{dm:.0f} ml Glucose 30% IV lent 5 min",
...
def ceftriaxone(poids: float, age: float) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    return {
        "dose_g": dg,
        "admin": "IV 3-5 min ou IM si VVP impossible",
        "note": "Ne pas attendre le médecin si purpura",
        "ref": "BCFI — Ceftriaxone",
    }, None
...
def litican(poids: float, age: float, atcd: list) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    if "Glaucome" in atcd or "Glaucome par fermeture de l'angle" in atcd:
        ci.append("Glaucome")
    if "Adénome prostatique" in atcd or "Rétention urinaire" in atcd:
    ...
    return {
        ...
        "dose_note": note,
        "voie": "IM profond",
        "freq": f"Si besoin après 4-6 h — max {LITICAN_DOSE_MAX_JOUR} mg/24h",
        "attention": "NE PAS INJECTER EN IV",
        "ref": "BCFI — Litican",
    }, None

def protocole_eva(eva: int, poids: float, age: float, atcd: list, gl: Optional[float] = None) -> dict:
    als = []
    recs = []
    imao = "IMAO (inhibiteurs MAO)" in atcd
    isrs = "Antidépresseurs ISRS/IRSNA" in atcd
    if imao:
        als.append("IMAO — Tramadol contre-indiqué")
    if isrs:
        als.append("ISRS/IRSNA — Tramadol déconseillé")
    ...
    if eva >= 7:
        ...
        if r4:
            recs.append({
                "p": "3",
                "nom": "Piritramide (Dipidolor) IV",
                "dose": f"{r4['dmin']}-{r4['dmax']}mg",
                "d": r4["admin"],
                "al": [],
                "ref": r4["ref"],
            })
